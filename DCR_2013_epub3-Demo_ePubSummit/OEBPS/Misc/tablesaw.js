...
(function() {
	...
	Table.prototype._getPrimaryHeaders = function() {
		return this.$table.find( "thead" ).children().filter( "[data-tablesaw-heading-ref]" ).eq( 0 ).find( "th" ); //Fix
	};
...
(function(){
	...
	var ColumnToggle = function( element ) {
		...
		this.headers = this.$table.find( "[data-tablesaw-heading-ref]" ).eq( 0 ).find( "th" ); //Fix
	...
	ColumnToggle.prototype.init = function() {
		...
		$( this.headers ).not( "td" ).each( function() {
			...
			if( priority && priority !== "persist" ) {
				...
				$("<label><input type='checkbox' checked='' />" + $this.text() + "</label>" ) //Fix
					.appendTo( $menu )
					.children( 0 )
			...
			_init: function(){
				...
				var addClassToTable = function(){
					...
					makeHeadsActionable = function( h , fn ){
						$.each( h , function( i , v ){
							var b = $( "<button class='" + classes.sortButton + "'></button>" ); //Fix
							b.on( "click" , { col: v } , fn );
							$( v ).wrapInner( b );
							b.append( "<span class='tablesaw-sortable-arrow'></span>" ); //Fix
						});
					},
					...
					addSwitcher = function( heads ){
						$switcher = $( '<div></div>' ).addClass( classes.switcher ).addClass( classes.tableToolbar ); //Fix
...
// Must fix
							...
							html.push( '<option' + ( isDefaultCol && !isDescending ? ' selected=""' : '' ) + ' value="' + j + '_asc">' + $t.text() + ' ' + ( isNumeric ? '&#x2191;' : '(A-Z)' ) + '</option>' ); //Fix
							html.push( '<option' + ( isDefaultCol && isDescending ? ' selected=""' : '' ) + ' value="' + j + '_desc">' + $t.text() + ' ' + ( isNumeric ? '&#x2193;' : '(Z-A)' ) + '</option>' ); //Fix
						});
...
//					heads = el.find( "thead [data-tablesaw-sortable-ref] th"); //Fix
...
;(function(){
	...
	function createMiniMap( $table ){
		...
		var $btns = $( '<div class="tablesaw-advance minimap"></div>' ),
			$dotNav = $( '<ul class="tablesaw-advance-dots"></ul>' ).appendTo( $btns ),
			hideDot = 'tablesaw-advance-dots-hide',
			$headerCells = $table.find( 'thead [data-tablesaw-heading-ref] th' ); //Fix
		...
		function showHideNav(){
			...
			$table.find( "thead [data-tablesaw-heading-ref] th" ).each(function(i){ //Fix
	...
	var S = {
		...
		init: function( table ) {
			var $table = $( table ),
				...
				modeVal = '',
				$switcher = $( '<div></div>' ).addClass( S.classes.main + ' ' + S.classes.toolbar ); //Fix
			...
			for( var j=0, k = S.modes.length; j<k; j++ ) {
				...
				html.push( '<option' +
					( isSelected ? ' selected=""' : '' ) +
					' value="' + S.modes[ j ] + '">' + Tablesaw.i18n.modes[ j ] + '</option>' ); //Fix
			}
			html.push( '</select></span></label>' );
